...
def flatten_horizons(mesh, horizon_list):
    modified_mesh = mesh
    list_borders = extract_border(mesh)
    nvert = mesh.nverts
    ncorn = mesh.ncorners
    ...
    for i in range(ncorn):
        ...
        #  fix borders
    for i in range(len(list_borders)):
        A[ncorn + i, list_borders[i]] = 1*10
        list_y[ncorn + i][0] = mesh.V[list_borders[i]][1]*10
    ...
    for hor in range(nb_horizons):
        nhoriz = len(horizon_list[hor])
            ...
            # Constraints on horizon vertices
        y0 = mesh.org(horizon_list[hor][0]) # id of the first point of the horizon
            # horizon
        for e in range(nhoriz):
...
if __name__ == '__main__' :
    ...
    flatten_horizons(mesh, [horizon, horizon1])
